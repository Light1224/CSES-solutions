...
// see this ->https://codeforces.com/blog/entry/143251

void solve() {
  long long n, a, b;
  cin >> n >> a >> b;

  // Invalid cases (same logic as yours)
  ...
  vector<long long> seq1(n), seq2(n);
  ...
  if (a >= 1 && b >= 1) {
    long long start = 2 * (a - 1);
    long long end = a + b - 1;

    if (start < n && end < n && start <= end) {
      rotate(seq2.begin() + start, seq2.begin() + start + 1,
             seq2.begin() + end + 1);
    }
  }
